...
namespace ACT_FFXIV_Aetherbridge.Test.LogLineParser
{
    ...
    public class FRLogLineParserTest
    {
        ...
        public void Parse_LogLine_AddedLoot_Standard()
        {
            ...
            Assert.AreEqual("Une paire de bottines YoRHa type 51: unité de soin a été ajoutée au butin.", logEvent.LogMessage);
        ...
        {
            const string logLine = @"[23:13:29.000] 00:0039:Impossible d'obtenir la  paire de bottines YoRHa type 51: unité paranormale .";
            var logEvent = _parser.Parse(new ACTLogLineEvent {LogLine = logLine});
            var lootEvent = logEvent.XIVEvent;
            Assert.IsNotNull(logEvent.Id);
            Assert.IsNotNull(logEvent.XIVEvent);
            ...
            Assert.AreEqual("Impossible d'obtenir la paire de bottines YoRHa type 51: unité paranormale.", logEvent.LogMessage);
        ...
        public void Parse_LogLine_GreedLoot_YouRoll()
        {
            ...
            Assert.AreEqual("Combatant One jette les dés (Cupidité) pour le rouleau d'orchestrion “Alien Manifestation”. 71!",
                logEvent.LogMessage);
        ...
        {
            const string logLine = @"[23:12:36.000] 00:0841:Vous jetez les dés (Besoin) pour le  rouleau d'orchestrion “Alien Manifestation”. 71!";
            var logEvent = _parser.Parse(new ACTLogLineEvent {LogLine = logLine});
            var lootEvent = logEvent.XIVEvent;
            Assert.IsNotNull(logEvent.Id);
            Assert.IsNotNull(logEvent.XIVEvent);
            ...
            Assert.AreEqual("Combatant One jette les dés (Besoin) pour le rouleau d'orchestrion “Alien Manifestation”. 71!",
	            logEvent.LogMessage);
            Assert.AreEqual("rouleau d'orchestrion “Alien Manifestation”", lootEvent.Item.SingularName);
            Assert.AreEqual(1, lootEvent.Item.Quantity);
            Assert.AreEqual(false, lootEvent.Item.IsHQ);
        ...
        {
            const string logLine = @"[17:37:35.000] 00:103e:John GilgameshSiren obtient une  paire de bottines YoRHa type 51: unité de soin .";
            var logEvent = _parser.Parse(new ACTLogLineEvent {LogLine = logLine});
            var lootEvent = logEvent.XIVEvent;
            Assert.IsNotNull(logEvent.Id);
            Assert.IsNotNull(logEvent.XIVEvent);
            ...
            Assert.AreEqual("John Gilgamesh obtient une paire de bottines YoRHa type 51: unité de soin.", logEvent.LogMessage);
        ...
        {
            const string logLine = @"[23:08:44.000] 00:1041:Elmo Dy'grunJenova jette les dés (Cupidité) pour le  masque YoRHa type 51: unité de mêlée . 82!";
            var logEvent = _parser.Parse(new ACTLogLineEvent {LogLine = logLine});
            var lootEvent = logEvent.XIVEvent;
            Assert.IsNotNull(logEvent.Id);
            Assert.IsNotNull(logEvent.XIVEvent);
            ...
            Assert.AreEqual("Elmo Dy'grun jette les dés (Cupidité) pour le masque YoRHa type 51: unité de mêlée. 82!", logEvent.LogMessage);
        ...
        {
	        const string logLine = @"[23:08:44.000] 00:1041:Frank Sun-d'azur jette les dés (Besoin) pour la  carte 9S. 67!";
	        var logEvent = _parser.Parse(new ACTLogLineEvent {LogLine = logLine});
	        var lootEvent = logEvent.XIVEvent;
	        Assert.IsNotNull(logEvent.Id);
	        Assert.IsNotNull(logEvent.XIVEvent);
